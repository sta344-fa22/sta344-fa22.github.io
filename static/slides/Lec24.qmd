...
$$ 
\begin{aligned}
\boldsymbol{v}_t &\sim \mathcal{N}(0,\boldsymbol{V}_t) \\
\boldsymbol{\omega}_t &\sim \mathcal{N}(0,\boldsymbol{W}_t) \\
\end{aligned}
$$
...
v_t &\sim \mathcal{N}(0,\, \sigma^2_v) \\
\omega_1 &\sim \mathcal{N}(0,\, \sigma^2_\omega) \\
\end{aligned}
$$
:::
::::
...
\epsilon_t(\boldsymbol{s}) &\stackrel{ind}\sim \mathcal{N}(0,\tau_{t}^2) \\
\\
\boldsymbol{\beta}_t & = \boldsymbol{\beta}_{t-1} + \boldsymbol{\eta}_t \\
\boldsymbol{\eta}_t &\stackrel{i.i.d.}\sim \mathcal{N}(0,\boldsymbol{\Sigma}_{\eta}) \\
\\
u_t(\boldsymbol{s}) &= u_{t-1}(\boldsymbol{s}) + w_t(\boldsymbol{s}) \\
w_t(\boldsymbol{s}) &\stackrel{ind.}{\sim} \mathcal{N}\left(\boldsymbol{0}, \Sigma_t(\phi_t, \sigma^2_t)\right)
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\boldsymbol{\beta}_{0} &\sim \mathcal{N}(\boldsymbol{\mu}_0, \boldsymbol{\Sigma}_0) \\
u_{0}(\boldsymbol{s}) &= 0
\end{aligned}
$$
:::
...
lm(t_1~elev, data=ne_temp)$residuals %>%
  geoR::variog(data=., coords=coords, uvec=seq(0,0.3, length.out=30), messages=FALSE) %>% 
