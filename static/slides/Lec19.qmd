...
where $\boldsymbol{w}$ is a spatial weights matrix.
...
normalize_weights( st_touches(nc[1:12,], sparse=FALSE) )
```

## NC SIDS & Moran's I

Lets start by using a normalized adjacency matrix for $\boldsymbol{w}$ (shared county borders).
...
where $w_t \sim \mathcal{N}(0,\sigma^2_w)$ and $|\phi| < 1$, then
...
In the case of time, both of these definitions result in the same multivariate distribution for $\boldsymbol{y}$.
...
$$ y(s)|\boldsymbol{y}(-s) \sim \mathcal{N}\left(\delta + \phi \frac{1}{|N(s)|}\sum_{s' \in N(s)} y(s'),~ \sigma^2 \right) $$
...
where
$$ \boldsymbol{\epsilon} \sim \mathcal{N}(0,\sigma^2 \, \boldsymbol{I}) $$

## A toy example

![](imgs/triangle_adj.png){fig-align="center" width="33%"}
...
$$ \boldsymbol{y} \sim \mathcal{N}((I-\phi \, \boldsymbol{W})^{-1} \boldsymbol{\delta},~\sigma^2 \left((I-\phi \, \boldsymbol{W})^{-1}\right) \left((I-\phi \, \boldsymbol{W})^{-1}\right)^{t})$$
-->
...
\begin{aligned}
  p\big(y(s_1),y(s_2)\big) 
    ...
    &\propto \exp\left(-\frac{1}{2\sigma^2} (\boldsymbol{y}-0)
      \begin{pmatrix} 
      1 & -\phi W_{12} \\
      -\phi W_{21} & 1
      \end{pmatrix}
...
$$ \boldsymbol{y} \sim \mathcal{N}(0,~\sigma^2 \, ((\boldsymbol{I}-\phi \boldsymbol{W})^{-1}) ((\boldsymbol{I}-\phi \boldsymbol{W})^{-1})^t )$$

* Conditional Autoregressive (CAR)

$$ y(s)|\boldsymbol{y}(-s) \sim \mathcal{N}\left(\sum_{s'} W_{s\,s'} \, y(s'),~ \sigma^2 \right) $$

$$ \boldsymbol{y} \sim \mathcal{N}(0,~\sigma^2 \, (\boldsymbol{I}-\phi \boldsymbol{W})^{-1})$$
...
$$
$$ 
\boldsymbol{\epsilon} \sim \mathcal{N}(\boldsymbol{0},\,\boldsymbol{D}_{\sigma^2}^{-1}) =  \mathcal{N}(\boldsymbol{0},\, \sigma^2 \boldsymbol{D}^{-1}) 
$$
...
$$
\boldsymbol{y} \sim \mathcal{N}\left(\boldsymbol{X}\boldsymbol{\beta}, (\boldsymbol{I} - \phi \boldsymbol{D}^{-1} \boldsymbol{A})^{-1} \sigma^2 \boldsymbol{D}^{-1} \big((\boldsymbol{I} - \phi \boldsymbol{D}^{-1} \boldsymbol{A})^{-1}\big)^t \right)
$$
...
* Conditional Model
$$ y(s_i)|\boldsymbol{y}_{-s_i} \sim \mathcal{N}\left(X_{i\cdot}\beta + \phi\sum_{j=1}^n \frac{W_{ij}}{D_{ii}} ~ \big(y(s_j)-X_{j\cdot}\beta\big),~ \sigma^2 D^{-1}_{ii} \right) $$
...
$$
\boldsymbol{y} \sim \mathcal{N}(\boldsymbol{X}\boldsymbol{\beta},~\Sigma_{CAR})
$$
...
$$
\boldsymbol{\Sigma} = \sigma^2 \, (\boldsymbol{D} - \phi \, \boldsymbol{W}) = \sigma^2~\begin{pmatrix}
1 & -\phi & 0 \\
-\phi & 2 & -\phi \\
0 & -\phi & 1 
\end{pmatrix}^{-1}
