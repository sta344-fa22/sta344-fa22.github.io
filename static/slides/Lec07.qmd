...
* Variance
  ...
  Var(X) &= E\Big(\big(X-E(X)\big)^2\Big) = E(X^2)-E(X)^2 \\
         &= \sum_x \big(x - E(X)\big)^2 \cdot P(X = x) \\
...
Cov(X,Y) &= E\Big(\big(X-E(X)\big)\big(Y-E(Y)\big)\Big) = E(XY)-E(X)E(Y) \\
       &= \sum_x \big(x - E(X)\big)\big(y - E(Y)\big) \cdot P(X = x, Y=y) \\
       &= \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} (x-E(X))(y-E(Y)) \cdot f(x,y) \; dx \; dy \\
\\       
...
1. The process has finite variance / second moment 
