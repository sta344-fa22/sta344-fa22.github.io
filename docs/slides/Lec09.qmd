...
![](imgs/Lec09/ar2_conditions.png){fig-align="center" width="50%"}
...
\boldsymbol\xi_t = \boldsymbol{\delta} + \boldsymbol{F} \, \boldsymbol\xi_{t-1} + \boldsymbol w_t
$$
where
...
$$
\begin{aligned}
\underset{p \times 1}{\boldsymbol\xi_t} &= [y_t, y_{t-1}, y_{t-2}, \ldots, y_{t-p+1}]' \\
...
```{r echo=FALSE}
par(mfrow=c(2,2))
pacf(ma_q[[2]], lag.max = 10, main=expression(paste(theta,"={-1.5}"          )))
pacf(ma_q[[3]], lag.max = 10, main=expression(paste(theta,"={-1.5, -1}"      )))
pacf(ma_q[[4]], lag.max = 10, main=expression(paste(theta,"={-1.5, -1, 2}"   )))
pacf(ma_q[[5]], lag.max = 10, main=expression(paste(theta,"={-1.5, -1, 2, 3}")))
```
