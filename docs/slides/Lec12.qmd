...
```{r echo=FALSE}
y = arima.sim(n=500, model=list(ar=c(1.3,-0.5), ma=c(0.5,0.2))) %>% unclass()
...
( arma22_brms = brm(
    ...
    chains=2, refresh=0, iter = 5000
) )
```
```{r echo=FALSE}
(arma22_brms = readRDS("prev/Lec11/arma22_brms.rds"))
```
...
arma22_ests = bind_rows(
  tibble(
    ...
    .value = c(1.3, -0.5, 0.5, 0.2)
  ),
  tibble(
    model = "Arima",
    .variable = c("ar[1]", "ar[2]", "ma[1]", "ma[2]"), 
    .value = c(1.3171,  -0.5142,  0.4332,  0.1651)
  ),
  tibble(
    model = "HR",
    .variable = c("ar[1]", "ar[2]", "ma[1]", "ma[2]"), 
    .value = c(1.35724, -0.54477, 0.42457, 0.17248)
  )
)
